...
MPL_CONFIG_DIR = PROJECT_ROOT / "tmp" / "mplconfig"
...
# FILES
# ============================================================
sim_file = PROJECT_ROOT / "data" / "generated" / "generated_data_2025-2050.csv"
ar6_zip_file = (
    PROJECT_ROOT
    / "data"
    / "inputs"
...
output_dir = PROJECT_ROOT / "output" / "global_ghg_emissions_pathways"
output_pdf = output_dir / "Fig_8.pdf"
output_png = output_dir / "Fig_8.png"
manuscript_pdf = PROJECT_ROOT / "manuscript" / "Fig_8.pdf"
...
# This intentionally spans the full sensitivity space, including all tested
# epsilon values; only the highlighted reference quartet below is fixed at epsilon=0.
envelope = (
...
for p in pathways:
    ...
    if pathway_key in seen_pathways:
        continue
    ...
    label = (
        ...
        f"$d_r$={pct_from_rate_int(p['d_rest'])}, "
        f"$\\varepsilon$={pct_from_fraction_1dec(p['epsilon'])})"
    )
...
manual_shift = {
    "S0 ($b$=5.5%, $g$=3%, $d_m$=1%, $d_r$=1%, $\\varepsilon$=0.0%)": -4,
    "S1 ($b$=5.5%, $g$=3%, $d_m$=1%, $d_r$=1%, $\\varepsilon$=0.0%)": -4,
    "S2 ($b$=5.5%, $g$=3%, $d_m$=1%, $d_r$=1%, $\\varepsilon$=0.0%)": -4,
    "S3 ($b$=5.5%, $g$=3%, $d_m$=1%, $d_r$=1%, $\\varepsilon$=0.0%)": -4,
    "S3 ($b$=5.5%, $g$=3%, $d_m$=1%, $d_r$=1%, $\\varepsilon$=1.5%)": 1,

    "S0 ($b$=5.5%, $g$=3%, $d_m$=3%, $d_r$=3%, $\\varepsilon$=1.5%)": 2.0,
    "S3 ($b$=5.5%, $g$=3%, $d_m$=3%, $d_r$=3%, $\\varepsilon$=1.5%)": 2.0,

    "S0 ($b$=3.3%, $g$=2%, $d_m$=5%, $d_r$=5%, $\\varepsilon$=0.0%)": -2.0,
    "S0 ($b$=5.5%, $g$=3%, $d_m$=1%, $d_r$=3%, $\\varepsilon$=1.5%)": 2.0,
    "S0 ($b$=3.3%, $g$=1%, $d_m$=7%, $d_r$=7%, $\\varepsilon$=0.0%)": 0.5,
}
...
for p in sim_points_adj:
    ax.plot([x_end, x_sim - 0.25], [p["y"], p["y_text"]], color=p["color"], lw=0.8)
    ax.text(
        x_sim,
        p["y_text"],
        p["label"],
...
for p in ssp_points_adj:
    ax.plot([x_end, x_ssp - 0.25], [p["y"], p["y_text"]], color=p["color"], lw=0.8)
    ax.text(
        x_ssp,
        p["y_text"],
        p["label"],
...
ssp_handles = [
    Line2D([0], [0], color=ssp_colors[lab], lw=1.8, ls=":", label=lab)
    for lab in ssp_order
]
...
# STYLING
# ============================================================
ax.axvline(2035, color="black", ls="--", lw=0.8, alpha=0.6)

ax.set_title("Global GHG emissions pathways to 2050", fontsize=18)
ax.set_xlabel("Year", fontsize=13)
ax.set_ylabel("Total global GHG emissions (GtCO$_2$e/yr)", fontsize=13)
ax.tick_params(axis="both", labelsize=13, width=0.5)

ax.set_xlim(2025, 2074)
ax.set_xticks([2025, 2030, 2035, 2040, 2045, 2050, 2060, 2070])
...
parser = argparse.ArgumentParser(
    description="Generate Figure 8: global GHG emissions pathways to 2050."
)
parser.add_argument(
    "--sync-manuscript",
    action="store_true",
    help="Copy the generated Figure 8 PDF into the manuscript directory.",
)
args = parser.parse_args()
if args.sync_manuscript:
    shutil.copy2(output_pdf, manuscript_pdf)
